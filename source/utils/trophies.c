    ...
    for (;;) {
        ...
        if (sceNpTrophyCreateHandle(&handle) >= 0) {
            ...
            if (rc < 0) {
                 ...
                 *
                 * I briefly made this opt-in after achievements were reported broken.
                 * That was wrong and the logs say so plainly: on this device the set
                 * is NOT registered, every sceNpTrophyUnlockTrophy returns
                 * 0x80551612, and without this recovery NOTHING unlocks at all. With
                 * it, the log shows the set registering (51 trophies) and the trophy
                 * unlocking. Turning it off does not restore old behaviour -- it
                 * removes the only thing making any trophy work. */
