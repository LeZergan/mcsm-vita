...
static TrackedRef *g_tracked_refs = NULL;
...
static TrackedRef *tracked_find_locked(jobject obj) {
    TrackedRef *entry = g_tracked_refs;
    while (entry) {
        if (entry->obj == obj) {
            return entry;
        }
...
static void tracked_free_object(jobject obj, TrackedObjKind kind) {
    ...
    switch (kind) {
        case TRACKED_OBJ_ARRAY:
            (void)jda_free((JavaDynArray *)obj);
            break;
...
static void tracked_register_local_owned(jobject obj, TrackedObjKind kind) {
    ...
    entry->next = g_tracked_refs;
    g_tracked_refs = entry;
    tracked_refs_unlock();
}

static void tracked_add_ref_if_known(jobject obj, int as_global) {
    if (!obj || is_reserved_fake_ref(obj) || !as_global) {
        return;
    }
...
static int tracked_release_ref_if_known(jobject obj, int prefer_global) {
    ...
    while (*pp) {
        ...
        if (entry->seen_global && entry->local_refs == 0 && entry->global_refs == 0) {
            *pp = entry->next;
            released = entry;
        }
        break;
...
jclass FindClass(JNIEnv* env, const char* name) {
    ...
    tracked_register_local_owned(clazz, TRACKED_OBJ_CLASS);
    return clazz;
}
...
void DeleteLocalRef(JNIEnv* env, jobject obj) {
    ...
    // Objects can still be referenced by Java-side state; keep local deletes as no-op.
}
